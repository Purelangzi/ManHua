      ...
      <div class="search">
        <el-form :model="Searchform" label-width="80px" :inline="true">
          <el-form-item label="角色名称">
            <el-input v-model.trim="Searchform.role_name" placeholder="请输入角色名" @keyup.enter.native="onSeach" />
          </el-form-item>
          <el-form-item>
            ...
            <el-button v-show="isBtnDisabled" type="primary" size="medium" @click="addList">新增角色</el-button>
    ...
    <div class="content">
      ...
      >
        <div class="drawer__content">
          <el-form ref="roleForm" :model="roleForm" :rules="roleFormRuels" style="width: 80%;">
            <el-form-item label="角色名称" prop="role_name">
              <el-input v-model="roleForm.role_name" placeholder="请输入角色名称" />
            </el-form-item>
            <el-form-item label="角色值" prop="role_value">
              <el-input v-model="roleForm.role_value" placeholder="请输入角色值" />
            </el-form-item>
            <el-form-item label="状态" class="statusStyle">
              <el-switch v-model="roleForm.role_status" active-text="已启用" inactive-text="已禁用" class="switchStyle" />
            </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { isSlot: false, prop: 'role_value', label: '角色值' },
  ...
  methods: {
    ...
    async getList() {
      ...
      const res = await this.$API['system'].getRoleList({ page, pageSize, role_name })
      if (res.code === 200) {
        this.totalNum = res.data.data.length !== 1 ? res.data.total : 1
        this.listData = res.data.data
        this.listData.forEach(el => {
          el.role_status = el.role_status === 0
        })
      }
      this.loading = false
    },
    ...
    async roleStatusChange(status, index) {
      const roleRow = this.listData[index]
      delete roleRow.sort
    ...
    submitAddOrUpdate() {
      this.$refs.roleForm.validate(async(valid) => {
        if (valid) {
          ...
          const flag = this.listTitle === 1
          this.roleForm.menus = this.treeData.menusId.join()
          const res = await this.$API.system[flag ? 'addRole' : 'editRole'](this.roleForm)
          if (res.code === 200) {
