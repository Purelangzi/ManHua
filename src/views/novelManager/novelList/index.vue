    ...
    <div class="header">
      <div class="search">
        <el-form ref="Searchform" :model="Searchform" label-width="80px" style="display: flex;" :inline="true">
          <el-form-item label="小说名称" style="min-width: 240px; display: flex;">
        ...
        <div class="abilityBtn">
          ...
          <el-upload
            ...
            :auto-upload="false"
          >
            <el-button type="primary" size="medium">一键导入当前小说列表的章节列表</el-button>
          </el-upload>
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      const res = await this.$API['novel'].getList({ page, pageSize, name })
      if (res.code === 200) {
    ...
    // 获取分类数据
    async getCategory() {
      const res = await this.$API['novel'].getCategory()
      if (res.code === 200) {
    ...
    async importExcChapter(file) {
      ...
      try {
        ...
        if (this.listData.length === 0) {
          this.logform.data = `小说信息匹配失败，请检查是否存在小说信息\n` + this.logform.data
          return
        }
        ...
        for (let i = 0; i < this.listData.length; i++) {
          ...
          for (let index = 0; index < listLength; index++) {
            ...
            if (el.comicId === item.platform_comic) {
              ...
              const res = await this.$API.novel.addChapter(params)
              if (res.code === 200) {
                this.logform.data = `${index + 1}/${listLength}小说章节导入成功\n` + this.logform.data
              } else {
                this.logform.data = `${index + 1}/${listLength}小说章节导入失败,${res.msg}\n` + this.logform.data
              }
            }
          }
          this.logform.data = `第${i + 1}小说章节导入完毕\n` + this.logform.data
        }
      } catch (error) {
    ...
    moreUpdateCategory() {
      ...
      this.listData.forEach(async(el) => {
        ...
        try {
          const { msg } = await this.$API.novel.updateList(el)
          this.logform.data += `${el.name} ${msg}\n`
        } catch (error) {
          this.logform.data += '修改失败\n'
        }
      })
      this.$message.success('当前页小说的批量修改分类成功')
    ...
    dropList(ids, name) {
      this.$confirm(`确定要${name ? '删除 ' + name : '批量删除'} 吗?`, '提示', {
        ...
        const data = await this.$API.novel.deleteList({ ids })
